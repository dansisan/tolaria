...
### Title edit from the body

Pressing Up arrow with a collapsed caret on the first line of the note body's first block opens the title (the breadcrumb filename) for editing instead of moving the caret. `createEditTitleOnArrowUpExtension` (a BlockNote extension on the editor) detects this with ProseMirror's `endOfTextblock('up')` plus a first-block check (`getTextCursorPosition().prevBlock === undefined`) — wrapped first paragraphs and lower blocks keep normal cursor movement — and dispatches `EDIT_NOTE_TITLE_EVENT`. `FilenameCrumb` listens for it and enters edit mode (only when the note is renameable). Enter or Down (Down mirrors Up-into-title) commit and hand focus back to the body via `laputa:focus-editor`; Escape cancels and returns focus; blur-submit leaves focus alone so clicking away doesn't yank the caret back. The decision is a pure helper, `shouldEnterTitleEditOnArrowUp` (`src/utils/editNoteTitleEvent.ts`). New-note creation also lands in the title: `laputa:focus-editor` carries an `editTitle` flag, and `useEditorFocus` dispatches `EDIT_NOTE_TITLE_EVENT` (after the tab swap for the new path) instead of focusing the body, so an untitled note opens with its filename selected for typing.
...
Orphaned image attachments are pruned on save. `save_note_content` diffs the previous on-disk content against the new content and returns the attachment links that were dropped; `persistContent` re-emits those as a `laputa:attachments-unlinked` event. `useAttachmentCleanup` (wired in `App`) listens for it, keeps any image still referenced by another note's `attachmentLinks` index (`src/utils/attachmentPruning.ts`), and deletes the rest via the `delete_attachment` command. Deletion is a git-recoverable `fs::remove_file`, mirroring note deletion, and is fenced to the `attachments/` directory.
...
- Obsidian-style fence entry: `createCodeFenceOnEnterExtension` (`src/components/codeFenceOnEnterExtension.ts`) converts a paragraph containing only ` ``` ` (optionally ` ```lang `) into a code block on plain Enter, resolving language aliases (`js` → `javascript`) against the same `supportedLanguages` map. It complements BlockNote's built-in input rule, which only fires on ` ``` ` + space.
...
- `plainTextPaste.ts` is the shared plain-text paste target registry. Rich BlockNote and raw CodeMirror surfaces register focused insertion targets, while ordinary focused text controls use DOM selection replacement, so the `Option+Cmd+V` command can preserve caret/selection behavior without each surface inventing its own clipboard reader.
...
Autosave then waits for a 30s idle window (`AUTO_SAVE_DEBOUNCE_MS`, ADR-0136) before invoking `save_note_content` — it is a crash backstop; interactive exits (note switch, raw-mode toggle, Cmd+S / File→Save, destructive actions) flush immediately. If an older save resolves after the user has already typed newer content, the older save is treated as stale and cannot clear the newer pending buffer or repaint tab state over it; the latest pending content remains scheduled for its own save.
...
interface Settings {
  ...
  note_list_description_property: string | null // frontmatter key a note can set; "" = off, null = default "description"
  note_list_preview_fallback_lines: number | null // 0–3 lines of note body for notes without one, null = default 1
  suggested_relationships: string | null // comma-separated relationship keys; "" = offer none, null = default "belongs_to, related_to, has"
  suggested_properties: string | null // comma-separated property labels; "" = offer none, null = default "Status, Date, URL, Icon, Aliases"
