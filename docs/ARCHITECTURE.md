...
The provider also carries the relationship vocabulary. `src/utils/suggestedRelationships.ts` resolves `suggested_relationships` into the key list that `useSuggestedRelationships()` publishes, and three surfaces read it: the Inspector's Relationships panel offers each unfilled key as a ready-to-fill slot, that panel and `usePropertyPanelState` use it to decide whether a key a Type schema declares without a wikilink value belongs to Relationships or to Properties, and `PropertyValueCells` uses it to pick the relationship icon. Those three must read one list or a schema key would appear in both panels or in neither. The vocabulary is presentation only: relationship *detection* stays dynamic (ADR-0010), so any frontmatter key whose value holds `[[wikilinks]]` is a relationship whether or not it is listed. A cleared list is persisted as `""` and leaves the panel with its "Add relationship" button alone.

`src/utils/suggestedProperties.ts` does the same for the Properties panel, resolving `suggested_properties` into `{key, label, mode}` triples that `DynamicPropertiesPanel` reads via `useSuggestedProperties()`. The triple keeps the write key separate from the label so a built-in survives any spelling — `Icon` still writes `_icon` and still opens the text editor — while a custom key gets `humanizePropertyKey` for its label and the plain text editor. `aliases` is listed but rendered by `AliasesRow` rather than as a bottom slot, because it needs the structural chip editor; the setting only decides whether that row appears while the note has no aliases. Note that aliases themselves are a resolution feature, not just a label: `findEntryByAlias` in `src/utils/wikilink.ts` resolves `[[alias]]` to the note, `linkTargetsForEntry` counts alias matches as backlinks, and wikilink autocomplete, person mentions, and the note-search dropdown all rank on them. The Rust full-text search in `src-tauri/src/search.rs` does not index aliases.
...
| File | Purpose |
|------|---------|
| `mod.rs` | Core types (`VaultEntry`, `Frontmatter`), `parse_md_file`, `scan_vault`, relationship/link extraction, `is_default_note_type` gate (Note vs. structured Type) |
| `parsing.rs` | Text processing: snippet extraction, markdown stripping, ISO date parsing, `extract_title` (H1 → legacy frontmatter → filename — used only for structured Type instances; `parse_md_file`/`mod.rs` short-circuits to the filename stem for plain Notes) |
...
| `rename.rs` | `rename_note_filename` / `move_note_to_folder` — stage crash-safe file moves (syncing a Type instance's `title` frontmatter when it mirrored the old filename stem), and recover unfinished rename transactions. Each returns a `PendingWikilinkRewrite` alongside the immediate result; the vault-wide backlink rewrite (and its failure counts) runs afterward via `PendingWikilinkRewrite::run`, not as part of the rename itself. This is the only rename operation — renaming a Note's file is retitling it (ADR-0138) |
...
| `rename_note_filename` | Crash-safe filename rename (syncing a Type instance's `title` frontmatter when it mirrored the old filename stem); async, returns as soon as the file move lands. Cross-vault wikilink rewrite + failed backlink counts run afterward in the background and are reported via `wikilinks-rewrite-completed`. This is the only rename operation — renaming a Note's file is retitling it (ADR-0138) |
| `move_note_to_folder` | Crash-safe folder move that preserves the filename and reloads the moved note; async for the same reason as `rename_note_filename` — the path-based wikilink rewrite is deferred and reported via `wikilinks-rewrite-completed` |
