...
// the editor. When inactive the fill is --surface-card, the list's own surface,
// so the row recedes to a plain tile carrying only the grey border rather than
// drawing attention away from the editor, which is where keystrokes now go.
// Both colours are type-independent, so the flip reads on every note.
